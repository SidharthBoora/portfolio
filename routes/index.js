...
router.get('/',(req,res,next)=>{
  insertModel.find((err,data)=>{
    res.render('index',{title:"Sidharth Boora",msg:"",user:data});
  })
});
...
router.post('/',checkEmail,(req,res,next)=>{
  const {name,email,subject,project}=req.body;
   ...
   conDetail.save((err,data)=>{
     if(err) throw err;
     insertModel.find((err,data)=>{
      res.render('index',{title:"Sidharth Boora",user:data,msg:"Your Record Saved Successfully"});
...
router.get('/login',(req,res,next)=>{
  ...
  res.redirect('/insert');
    ...
    res.render('login',{title:"Sidharth Boora",msg:""});
  }
});
...
router.post('/login',(req,res,next)=>{
  ...
  checkuesr.exec((err,data)=>{
    if(err) throw err;
    if(data==null){
      res.render('login',{title:'Sidharth Boora',msg:"Invalid Username Or Password"});
    }else{
...
router.get('/insert',checkLogin,(req,res,next)=>{
  res.render('insertdata',{title:"Sidharth Boora",msg:""});
});
